...
/*   Updated: 2024/03/29 18:40:08 by jhoratiu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	find_path_cbles(t_complete *s, int x, int y, int jump)
{
	if(s->map_copy[y + 1][x] == '1'
		|| s->map_copy[y][x + 1] == '1'
		|| s->map_copy[y][x - 1] == '1')
		check_path_cbles(s, x, y - 1, jump + 1);
...
void	find_path_exit(t_complete *s, int x, int y, int jump)
{
	if(s->map_copy[y + 1][x] == '1')
		jump == 0;
	check_path_exit(s, x + 1, y, jump);
	check_path_exit(s, x - 1, y, jump);
	check_path_exit(s, x, y + 1, jump);
	if(jump == 0)
		check_path_exit(s, x, y - 1, jump + 1);
}
